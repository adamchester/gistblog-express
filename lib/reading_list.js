	...
	*/
	...
	function extractTags(readingListItems) {
		return _.chain(readingListItems)
			...
			.flatten()
			.groupBy(c.self)
			.map(function (group) { return { tagName: group[0], count: group.length }; })
			.sortBy(c.self)
			.map(toTagItem)
			.value();
	}
	...
	function toTagItem(item) {
		// TODO: move this into something more 'presentation' oriented?
