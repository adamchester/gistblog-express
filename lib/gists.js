...
var Gists = (function() {
	...
	function getBlogPostsForUser(options, callback) {
		...
		requestJson(url, function (error, json) {
			...
			if (error) {
				// Error
				log.error('getBlogPostsForUser: error getting gists for user [%s], %s', options.username, error);
				callback(error);
			} else {
				var hasValidGistsList = json !== null && json.length > 0;
				if (!hasValidGistsList) {
					log.error('getBlogPostsForUser: got an empty gist list');
					...
					if (getAllContents) {
						...
						asyncPopulateAllContentsForPosts(posts, function(error, postsWithContent){
							...
							if (!error) {
								...
								log.error('getBlogPostsForUser: got asyncError %s', asyncError);
								callback(error);
							}
						});
					} else {
