...
function CacheBlockFirstThenAsync(fn, cacheOptions) {
	...
	function refreshCachedValueAsync() {
		...
		if (_hasArgs) {
			opt.logInfo('calling cached method with args = %s', dumpObjectToString(opt.args));
		}
		else {
			opt.logInfo('calling cached method (no args)');
		}

		if (opt.args) {
			fn(opt.args, onFnCallback);
		}
		else {
			fn(onFnCallback);
		}
