...
(function() {
  ...
  var blogPostsForUserCache = cache.cachify(gists.getBlogPostsForUser, {
    args: getArgs, expiryMinutes: 10.0 });
  ...
  */
  ...
  function TopLevelPage(id, name, href, title, classes) {
    this.id = id;
  ...
  TopLevelPage.prototype.activeIfEquals = function activeIfEquals(topLevelPage) {
    return this === topLevelPage ? 'active' : null;
  };

  var topLevelPages = {
    none: new TopLevelPage('none', 'none', '', 'none'),
    index: new TopLevelPage('index', 'Posts', '/', 'Rarely updated blog', ['glyphicon', 'glyphicon-inbox']),
    about: new TopLevelPage('about', 'About', '/about', 'About the Rarely Updated blog', ['glyphicon', 'glyphicon-glass']),
    reading: new TopLevelPage('reading', 'Reading', '/reading', 'Reading list', ['glyphicon', 'glyphicon-book']),
    twitter: new TopLevelPage('twitter', 'Twitter', '/twitter', 'Twitter - not quite as rarely updated', ['glyphicon', 'glyphicon-retweet']),
    feed: new TopLevelPage('feed', 'Feed', '/rss', 'The rarely updated feed'),
  };
  ...
  function getTopLevelMenuItems(renderingTopLevelPage) {
    return _.chain(topLevelPages)
      ...
      .filter(c.pageIsNot(topLevelPages.feed))
  ...
  //
  // View model for GET /rss
  //
  function buildFeedModel(posts) {
    
    var page = topLevelPages.feed;
    return new ViewModel(page.title, page.pageTemplateName)
      .extend({
        posts: posts
      });
  ...
  function getFeedModel(callback) {
    async.parallel({
        posts: asyncify(blogPostsForUserCache.get)
      },
      function gotFeedModelData(err, results) {
        if (err) { return callback(err); }

        var viewModel = buildFeedModel(results.posts);
        callback(err, viewModel);
      });
  }
  ...
  Shared.getFeedModel = getFeedModel;
}());
